...
{	
    ...
    GPIO_Init(GPIO_I2C, GPIO_SDA,   GPIO_Mode_In_FL_No_IT );
    GPIO_Init(GPIO_I2C, GPIO_SCL,   GPIO_Mode_Out_PP_High_Fast );
...
void FDC_IIC_Start(void)
{
	...
	FDC_IIC_SCL_1;
	FDC_IIC_Delay();
 	FDC_IIC_SDA_0;//START:when CLK is high,DATA change form high to low 
	FDC_IIC_Delay();
	FDC_IIC_SCL_0;//钳住I2C总线，准备发送或接收数据 
}	  
//产生IIC停止信号
void FDC_IIC_Stop(void)
{
	FDC_SDA_OUT();//sda线输出
	FDC_IIC_SCL_0;
	FDC_IIC_SDA_0;//STOP:when CLK is high DATA change form low to high
 	FDC_IIC_Delay();
	FDC_IIC_SCL_1; 
	FDC_IIC_SDA_1;//发送I2C总线结束信号
	FDC_IIC_Delay();							   	
}
//等待应答信号到来
//返回值：1，接收应答失败
...
u8 FDC_IIC_Wait_Ack(void)
{
	u8 ucErrTime=0;
	FDC_SDA_IN();      //SDA设置为输入  
	FDC_IIC_SDA_1;FDC_IIC_Delay();	   
	FDC_IIC_SCL_1;FDC_IIC_Delay();	 
	while(GPIO_ReadInputDataBit(GPIO_I2C,GPIO_SDA))
	{
		ucErrTime++;
		if(ucErrTime>250)
		{
...
u8 FDC_IIC_Read_Byte(unsigned char ack)
{
    ...
    for(i=0;i<8;i++ )
	{
		...
		FDC_IIC_SCL_1;
        receive<<=1;
        if(GPIO_ReadInputDataBit(GPIO_I2C,GPIO_SDA))receive++;   
		FDC_IIC_Delay(); 
    }				 
    if (!ack)
        FDC_IIC_NAck();//发送nACK
